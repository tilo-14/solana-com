...
<Callout>
  ...
  payment concepts. See here for payment flows with [Light Token](https://www.zkcompression.com/light-token/payments/overview).
</Callout>

The `splToken` helper from
[`@solana/client`](https://solana.com/docs/frontend/client) handles ATA
...
<Callout>
  See here, for similar payment flows using [Light Token](https://www.zkcompression.com/light-token/payments/overview).
</Callout>
...
- **ATA Resolution**: Automatically derives Associated Token Accounts (ATAs) for
  ...
  transaction.
- **Decimal Conversion**: Accepts token amounts and automatically converts to
  base units based on the mint's decimals (e.g. 0.25 tokens -> 250000 base
  units, if the mint has 6 decimals)
- **Transaction Building**: Creates, signs, and sends the transaction with the
