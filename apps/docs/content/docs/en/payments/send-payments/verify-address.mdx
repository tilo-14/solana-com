...
Token Program, Token-2022, or Light Token Program and hold token balances. 
If the address you receive is owned by a token program, you should verify the account is a token
...
<Callout type="info">
Light Token accounts can hold balances of SPL, Token 2022, and Light Token mints.
</Callout>
...
- **Token Program / Token-2022 / Light Token**: Verify the account is a token account (not a
...
<Callout>
  ...
  example code.
  See here, for address validation when using [Light Token](https://www.zkcompression.com/light-token/payments/overview).
</Callout>
