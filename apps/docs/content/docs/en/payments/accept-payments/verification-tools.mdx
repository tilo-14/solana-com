...
<Callout>
  When using [Light Token](https://www.zkcompression.com/light-token/payments/overview), use `getAtaInterface` instead.
</Callout>
...
<Callout>
  When using [Light Token](https://www.zkcompression.com/light-token/payments/overview), use `rpc.getSignaturesForOwnerInterface()` instead.
</Callout>
